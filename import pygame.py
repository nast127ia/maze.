...
class Player:
    ...
    def check_collision(self, x, y):
        if level[int((y + self.height) // BLOCK_SIZE)][int((x + self.width) // BLOCK_SIZE)] == "F":
            return True
        return False
...
class Game:
    def __init__(self):
        ...
        self.show_congrats = False  # Додано прапорець для показу вітання
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_a and self.show_lobby:
                        ...
                        pygame.mixer.music.play(-1)  # Відтворення музики
